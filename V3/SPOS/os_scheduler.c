...
SchedulingStrategy currSchedulingStr = OS_SS_EVEN;

//! Count of currently nested critical sections
uint16_t critiSectionCount = 0;
...
ISR(TIMER2_COMPA_vect) 
{
	...
	os_processes[os_getCurrentProc()].hashwert = os_getStackChecksum(os_getCurrentProc());
	
	if ((os_getInput() & 0b1001) == 0b1001)
	{
		os_waitForNoInput();
		os_taskManMain();
	}
	...
	{
		case OS_SS_EVEN:
		currProc = os_Scheduler_Even(os_processes, currProc);
		break;
		
		case OS_SS_RANDOM:
		currProc = os_Scheduler_Random(os_processes, currProc);
		break;
		/************************************************************************/
		/*case OS_SS_RUN_TO_COMPLETION:
		currProc = os_Scheduler_RunToCompletion(os_processes, currProc);
		break;
		case OS_SS_ROUND_ROBIN:
		currProc = os_Scheduler_RoundRobin(os_processes, currProc);
		break;
		case OS_SS_INACTIVE_AGING:
		currProc = os_Scheduler_InactiveAging(os_processes, currProc);
		break;                                                                 */
		/************************************************************************/
		default:
		currProc = os_Scheduler_Even(os_processes, currProc);
	...
	if (os_processes[os_getCurrentProc()].hashwert == os_getStackChecksum(os_getCurrentProc()))
	{
...
{    
	...
	for(uint8_t procID=0; procID<MAX_NUMBER_OF_PROCESSES; procID++)
	{
		 if(os_processes[procID].state == OS_PS_UNUSED)
		 {
			 Program* progAddress = os_lookupProgramFunction(programID);
			 ...
			 if(progAddress == NULL)
			 {
				...
				// belongs to present task os_resetProcessSchedulingInformation(procID);
				os_processes[procID].state = OS_PS_READY;
				os_resetProcessSchedulingInformation(procID);
				os_processes[procID].priority = priority;
				os_processes[procID].progID = programID;
				
				uint8_t lowByte = (uint16_t)(progAddress) & 0b11111111;
				uint8_t highByte =(((uint16_t)(progAddress) & ~(0b11111111)) >> 8);
				
				os_processes[procID].sp.as_int = PROCESS_STACK_BOTTOM(procID);
				
				*(os_processes[procID].sp.as_ptr) = lowByte;
				os_processes[procID].sp.as_ptr--;
				*(os_processes[procID].sp.as_ptr) = highByte;
				os_processes[procID].sp.as_ptr--;
				
				for(uint8_t reg = 0; reg<33; reg++)
				{
					*(os_processes[procID].sp.as_ptr) = 0;
					os_processes[procID].sp.as_ptr--;
				}
				
				os_processes[procID].hashwert = os_getStackChecksum(procID);
...
void os_initScheduler(void) 
{
	for (uint8_t procID = 0; procID < MAX_NUMBER_OF_PROCESSES; procID++)
	{
		os_processes[procID].state = OS_PS_UNUSED;
	}
...
/*!
 *  This function return the the number of currently ready process-slots.
 *
 *  \returns The number currently ready process-slots.
 */
uint8_t os_getNumberOfReadyProcs(void) {
    uint8_t num = 0;

    ProcessID i = 1;
    do {
        num += os_getProcessSlot(i)->state == OS_PS_READY;
    } while (++i < MAX_NUMBER_OF_PROCESSES);
...
{
	uint8_t bitMaske = 0b10000000;
	uint8_t gieb = SREG & bitMaske;
	SREG &= ~bitMaske;
	critiSectionCount++;
	TIMSK2 &= 0b11111101;
	SREG |= gieb; 
...
void os_leaveCriticalSection(void) 
{
	if (critiSectionCount > 0)
	{
		...
		if (critiSectionCount == 0) TIMSK2 |= 0b10;
		
		SREG |= gieb;
		
	} else if(critiSectionCount == 0) lcd_writeString("No process in CS");
	  else os_error("error in CS - more leave than inter");
}
...
{
	StackChecksum sc = 0;
	StackPointer posOfStack;
	posOfStack.as_int = PROCESS_STACK_BOTTOM(pid);
	while (posOfStack.as_ptr != os_processes[pid].sp.as_ptr)
	{
		sc ^= *(posOfStack.as_ptr);
		posOfStack.as_ptr--;
	}
