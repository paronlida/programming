 ...
 */
void performSchedulabilityTest(SchedulingStrategy strategy) {
    ...
    // Find the first not schedulable process id
    uint8_t firstUnschedulable = 0;
    for (uint8_t k = 1; k < MAX_NUMBER_OF_PROCESSES; k++) {
        if (!(captured & (1 << k))) {
            firstUnschedulable = k;
            break;
        }
    }
    ...
    lcd_goto(1, 14);
    lcd_writeDec(firstUnschedulable);
    lcd_line2();
...
PROGRAM(1, AUTOSTART) {
    ...
    for (k = 0; k < numStrategies; k++) {
        performSchedulabilityTest(strategies[k]);
    }
